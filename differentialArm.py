...
class DifferentialArms():
    ...
    def __init__(self, port, one_rotation_angle=64, close_direction=1,speed=400,start_position=0):
        """
        port: motor port
        ...
        self.speed=speed
        start_angle = int((2.5 if start_position==2 else 1 if start_position==1 else 0)*one_rotation_angle*close_direction)
        print(start_angle)
    ...
    async def set_closed_arms(self, arm, speed=400):
        if arm==self.ARM1:
            ...
            await self._moveArm(2.5,self.close_direction,speed)
    ...
    async def set_partial_closure(self, n, both_arms=True,speed=400):
        await diff_arms._moveArm(n*(2.5 if both_arms else 1),self.close_direction,speed=speed)
...
async def main():
    ...
    await diff_arms.set_closed_arms(0)
    
    moveWithGyro(-500,0,450)
    
    await diff_arms.set_partial_closure(0.9)

    runloop.sleep_ms(1000)
    moveWithGyro(-200,0,850)

    await diff_arms.set_closed_arms(0)

    moveWithGyro(300,0,650)

    await diff_arms.set_closed_arms(2,speed=200)
